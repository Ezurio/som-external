#####################################################################
# Summit Remote Control Manager (RCM) Unauthenticated Plugin
#####################################################################

SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_VERSION = local
SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_SITE = $(BR2_EXTERNAL_SUMMIT_SOM_PATH)/externals/summit-rcm/summit_rcm/plugins/unauthenticated
SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_SITE_METHOD = local
SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_LICENSE = Ezurio
SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_LICENSE_FILES = LICENSE.ezurio
SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_SETUP_TYPE = pep517
SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_DEPENDENCIES = python3 summit-rcm

ifeq ($(BR2_PACKAGE_HOST_PYTHON_CYTHON),y)
SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_DEPENDENCIES += host-python-cython
endif

SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_EXTRA_PACKAGES = \
	summit_rcm_unauthenticated \
	summit_rcm_unauthenticated/services \
	summit_rcm_unauthenticated/middleware

ifeq ($(BR2_PACKAGE_SUMMIT_RCM_REST_API_V2_ROUTES),y)
    SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_EXTRA_PACKAGES += summit_rcm_unauthenticated/rest_api/v2/system
endif
ifeq ($(BR2_PACKAGE_SUMMIT_RCM_REST_API_LEGACY_ROUTES),y)
    SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_EXTRA_PACKAGES += summit_rcm_unauthenticated/rest_api/legacy
endif

ifeq ($(BR2_PACKAGE_SUMMIT_RCM_REST_API_DOCS),y)
	SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_EXTRA_PACKAGES += summit_rcm_unauthenticated/rest_api/utils/spectree
	SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_DEPENDENCIES += host-summit-rcm-unauthenticated-plugin
	HOST_SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_DEPENDENCIES += \
		host-summit-rcm \
		host-python3 \
		host-python-falcon \
		host-python-spectree \
		host-python-pydantic \
		host-python-typing-extensions
	HOST_SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_ENV = \
		DOCS_GENERATION='True' \
		OPENAPI_JSON_PATH='$(TARGET_DIR)/summit-rcm-openapi-unauthenticated-plugin.json' \
		SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_EXTRA_PACKAGES='$(SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_EXTRA_PACKAGES)'
	HOST_SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_BUILD_OPTS = && \
		$(HOST_SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_ENV) \
		$(HOST_DIR)/bin/python3 generate_docs.py
endif

SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_ENV = SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_EXTRA_PACKAGES='$(SUMMIT_RCM_UNAUTHENTICATED_PLUGIN_EXTRA_PACKAGES)'

$(eval $(python-package))
$(eval $(host-python-package))
