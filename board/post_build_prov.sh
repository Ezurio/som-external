...
case "${BUILD_TYPE}" in
    *am6*)
        if ! grep -qF "BR2_PACKAGE_SUMMIT_PROV=y" "${BR2_CONFIG}"; then
            touch "${BINARIES_DIR}/prov_data.tar.gz_sign_enc.bin"
            exit 0
        fi
        ...
        tar -C "${prov_data_path}" -cf - . | zstd -fo "${BINARIES_DIR}/prov_data.tar.gz"
        ...
        if [ -n "${SECURE_TARGET_BUILD}" ]; then
            ...
            "${BR2_EXTERNAL_SUMMIT_SOM_PATH}/board/carbon/scripts/gen_core_x509_cert.sh" \
                -b "${BINARIES_DIR}/prov_data.tar.gz" \
                -k "${smpk_path}" \
                -a 2 \
                -n \
                -y ENCRYPT \
                -e "${smek_path}" \
                -o "${BINARIES_DIR}/cert_prov_data.tar.gz.bin"
            cat "${BINARIES_DIR}/cert_prov_data.tar.gz.bin" "${BINARIES_DIR}/prov_data.tar.gz-ENC" > "${BINARIES_DIR}/prov_data.tar.gz_sign_enc.bin"
            shred -zn 0 "${BINARIES_DIR}/cert_prov_data.tar.gz.bin" "${BINARIES_DIR}/prov_data.tar.gz-ENC"
        else
            # If not a secure target build, encrypt the prov_data.tar.gz using SMEK and inject the SMEK
            ...
            openssl enc -aes-256-cbc -in "${BINARIES_DIR}/prov_data.tar.gz" -out "${BINARIES_DIR}/prov_data.tar.gz_sign_enc.bin" -K "${KEY}" -iv "${IV}"
        ...
        shred -zn 0 "${BINARIES_DIR}/prov_data.tar.gz"

        # Verify the encrypted provisioning data file is not greater than 1MB
        if [ "$(stat -c%s "${BINARIES_DIR}/prov_data.tar.gz_sign_enc.bin")" -gt 1048576 ]; then
